package org.gaohui.message.utils;
...
public class WebSocketUtil {
     ...
     * @param <T>     消息类型
     */
    public static <T extends Message> void broadcast(String type, T message) throws Exception {
        // 将公共消息存储在数据库中未在线的成员上线之后http请求获得
        // 创建消息
     ...
     * @return 发送是否成功你那个
     */
    public static <T extends Message> boolean send(String toUser, String type, T message) throws Exception {
        // 获得用户对应的 Session
        WebSocketSession session = USER_SESSION_MAP.get(toUser);
        // 发送消息
